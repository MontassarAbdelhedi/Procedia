...
Description: Creates a null layer in the hosting comp, used as a general-purpose transform controller and parent anchor. Other layers wire into it to declare AE parenting relationships.
...
- layer*in*{n}: layer — unlimited. Each connected layer becomes a child
  of this null via AE layer parenting (layer.parent).
  Multiplicity: unlimited, same pattern as CompNode.
...
1. ARCHITECTURE CORRECTION — NullNode classification:
   The architecture doc lists NullNode as "Not-Dedicated". This is wrong. NullNode IS a Dedicated node. Update Section 1a of PROCEDIA-V2-ARCHITECTURE.md to reflect:
...
5. PARENTING — when a layer wire connects into layer*in*{n}, call setLayerParent(childUUID, nullUUID, hostingCompUUID)  immediately.
   When that wire is deleted, call clearLayerParent(childUUID, hostingCompUUID) immediately. The null itself must already be alive in the comp before any child can be parented to it. Never attempt
   setLayerParent before the null layer exists in AE.

6. 3D LAYER — not in v1.0.0. Do not add a 3D toggle param. Do not call
   layer.threeDLayer. Leave a comment in the node file:
   // TODO v1.1.0 — add 3D toggle param and z-position/rotation props

7. PROPERTY MATCH NAMES — use these exact AE match names, no exceptions:
...
8. ES3 STRICT — position and scale are arrays. Do NOT use array
   destructuring or spread. Access values by index:
   value[0], value[1]. Pass as AE array: [x, y].

9. PORT ORDERING — input ports layer*in*{n} appear first visually.
   The single output port appears last. Match CompNode port pattern
