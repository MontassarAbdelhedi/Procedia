...
Create every directory and empty placeholder file listed in Architecture §13.
Update `index.html` with the full `<script>` load order (empty bodies — just the tags).
**Verify:**
- [ ] All directories exist on disk
- [ ] `index.html` loads in CEP panel without console errors
...
No dependencies.
**Verify:**
- [ ] `generateNodeId()` called in console returns correct format string
...
**Verify:**
- [ ] `evalBridge('JSON.stringify({ok:true,data:"hello",error:null})')` resolves with `{ok:true, data:"hello"}`
- [ ] Malformed return string rejects the promise and logs parse error
...
**Verify:**
- [ ] `JSON.stringify({ok:true})` inside AE Script Editor returns `'{"ok":true}'`
- [ ] `JSON.parse('{"ok":true}')` returns object with `ok === true`
...
**Verify:**
- [ ] `graphState.addNode({id:'PROC-test', type:'TextNode',...})` — node appears in `nodeMap`
- [ ] `graphState.rebuildTempGraph()` — tempGraph reflects nodeMap contents
...
**Verify:**
- [ ] `worldToScreen(0, 0)` returns canvas center when pan is 0
- [ ] `screenToWorld(worldToScreen(100, 200))` returns `{x:100, y:200}`
...
**Verify:**
- [ ] Panel shows a grid that stays fixed as you pan (grid moves with viewport)
...
**Verify:**
- [ ] Middle-click drag pans the grid
- [ ] Scroll wheel zooms in/out centered on cursor
- [ ] Viewport coords are correct after pan + zoom combination
...
**Verify:**
- [ ] Canvas fills panel on load
- [ ] Canvas resizes correctly when panel is resized in AE
...
**Verify:**
- [ ] Register a dummy node, `lookup()` returns it
- [ ] `getCategories()` lists registered categories
...
**Verify:**
- [ ] A manually-added node entry in nodeMap renders visibly on canvas
- [ ] Clicking the node area returns true from `hitTestNode`
...
**Verify:**
- [ ] `nodeRegistry.lookup('CompNode')` returns definition with correct fields
...
**Verify:**
- [ ] `nodeRegistry.lookup('TextNode')` returns definition with all input ports listed
...
**Verify:**
- [ ] Manually add a wire entry to wireMap and confirm it draws on canvas between two points
...
**Verify:**
- [ ] Dragging from output port shows preview bezier following cursor
- [ ] Dropping on incompatible port type cancels (no wire created)
- [ ] Connecting A→B→A is rejected (cycle)
- [ ] Valid connection creates entry in wireMap
...
**Verify:**
- [ ] Manually build a nodeMap+wireMap chain `Text→Comp`; `hasCompDownstream('Text-uuid')` returns true
- [ ] Break the wire; `hasCompDownstream` returns false
...
**Verify:**
- [ ] Wire deleted in chain `Text→Effect→Comp`: effect removed first, then text parked (check AE layer order in timeline)
- [ ] Multi-comp: deleting one path leaves node alive in remaining comp
...
**Verify:**
- [ ] All registered node types appear grouped by category
- [ ] Typing in search hides non-matching entries in real time
- [ ] Category header click collapses/expands the group
...
**Verify:**
- [ ] Drag a TextNode onto canvas → entry appears in nodeMap with state='ghost'
- [ ] Drag a CompNode onto canvas → entry appears with state='alive', `makeCompAlive` called
- [ ] Node renders on canvas at correct world position
...
**Verify:**
- [ ] Select a ghost node, press Delete → node removed from nodeMap and canvas
- [ ] Select an alive node, press Delete → onGhost runs first, then node removed
...
5. `rebuildTempGraph()`
**Verify:**
- [ ] Drop TextNode → nodeMap entry with state='ghost', no AE call
- [ ] Drop CompNode → nodeMap entry with state='alive', AE creates comp
...
**Verify:**
- [ ] Wire TextNode to CompNode → TextNode state becomes 'alive', layer appears in AE comp
...
- See Architecture §14 rule 15: use moveToBeginning, never moveTo(index)
**Verify (AE Script Editor):**
...
Implement one wrapper function per ExtendScript command listed in Architecture §12.
...
- T0.3 (evalBridge) must precede all T7–T11 ExtendScript tasks
