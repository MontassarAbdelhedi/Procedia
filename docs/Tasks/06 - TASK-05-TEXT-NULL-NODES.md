...
Identical structure to TextNode — source node, no input ports, same three ports.
...
(function() {
  ...
  function testNode(defType, expectedLabel, expectedCategory, expectedOnAliveAction,
    ...
    for (var j = 0; j < def.ports.length; j++) {
      if (def.ports[j].category === 'input') hasInputPort = true;
    }
...
**No new dispatcher actions are introduced in this task.** All four action names used — `createTextLayer`, `createNullLayer`, `parkLayer`, `deleteParkedLayer`, `setLayerProperty` — are already declared in the dispatcher's registered actions table in `PROCEDIA-V4-ARCHITECTURE.md` Section 5b. Do not invent new names.
