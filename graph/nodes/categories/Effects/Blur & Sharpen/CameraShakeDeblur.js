/**
 * @file Camera Shake Deblur blur node definition (blur-sharpen/camera-shake-deblur).
 * An effector that applies the ADBE CameraShakeDeblur effect to a layer.
...
// graph/nodes/categories/Effects/Blur & Sharpen/CameraShakeDeblur.js
// DEPENDS ON: graph/nodeRegistry.js
// MUST LOAD BEFORE: index.js

var CameraShakeDeblurNode = {
  ...
  /**
   * Applies the ADBE CameraShakeDeblur effect to the upstream layer.
  ...
  /**
   * Removes the ADBE CameraShakeDeblur effect from the upstream layer.
   * @param {Object} nodeData - The full node data object.
   * @param {string} hostingCompUUID - UUID of the hosting composition.
  ...
  /**
   * Updates a single property on the ADBE CameraShakeDeblur effect in AE.
  ...
  onPropertyChange: function(key, value, nodeData, hostingCompUUID, upstreamNodeUUID) {
    return {
      ...
      params: {
        ...
        hostingCompUUID: hostingCompUUID,
        layerNodeUUID:   upstreamNodeUUID,
        effectMatchName: 'ADBE CameraShakeDeblur',
        propMatchName:   key,
        value:           value
      }
    };
...
nodeRegistry.register(CameraShakeDeblurNode);
