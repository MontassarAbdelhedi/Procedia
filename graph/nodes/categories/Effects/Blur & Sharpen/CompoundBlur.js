/**
 * @file Compound Blur blur node definition (blur-sharpen/compound-blur).
 * An effector that applies the ADBE Compound Blur effect to a layer.
...
// graph/nodes/categories/Effects/Blur & Sharpen/CompoundBlur.js
// DEPENDS ON: graph/nodeRegistry.js
// MUST LOAD BEFORE: index.js

var CompoundBlurNode = {
  ...
  /**
   * Applies the ADBE Compound Blur effect to the upstream layer.
  ...
  /**
   * Removes the ADBE Compound Blur effect from the upstream layer.
   * @param {Object} nodeData - The full node data object.
   * @param {string} hostingCompUUID - UUID of the hosting composition.
  ...
  /**
   * Updates a single property on the ADBE Compound Blur effect in AE.
  ...
  onPropertyChange: function(key, value, nodeData, hostingCompUUID, upstreamNodeUUID) {
    return {
      ...
      params: {
        ...
        hostingCompUUID: hostingCompUUID,
        layerNodeUUID:   upstreamNodeUUID,
        effectMatchName: 'ADBE Compound Blur',
        propMatchName:   key,
        value:           value
      }
    };
...
nodeRegistry.register(CompoundBlurNode);
