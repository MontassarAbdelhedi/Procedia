/**
 * @file Smart Blur blur node definition (blur-sharpen/smart-blur).
 * An effector that applies the ADBE Smart Blur effect to a layer.
...
// graph/nodes/categories/Effects/Blur & Sharpen/SmartBlur.js
...
var SmartBlurNode = {
  ...
  /**
   * Applies the ADBE Smart Blur effect to the upstream layer.
  ...
  /**
   * Removes the ADBE Smart Blur effect from the upstream layer.
   * @param {Object} nodeData - The full node data object.
   * @param {string} hostingCompUUID - UUID of the hosting composition.
  ...
  /**
   * Updates a single property on the ADBE Smart Blur effect in AE.
  ...
  onPropertyChange: function(key, value, nodeData, hostingCompUUID, upstreamNodeUUID) {
    return {
      ...
      params: {
        ...
        hostingCompUUID: hostingCompUUID,
        layerNodeUUID:   upstreamNodeUUID,
        effectMatchName: 'ADBE Smart Blur',
        propMatchName:   key,
        value:           value
      }
    };
...
nodeRegistry.register(SmartBlurNode);
