...
2. `arch_specs.md` — Section 5 (Centralized ExtendScript Dispatcher) in full
...
- `bridge/evalBridge.js` — the only door between the panel and After Effects. Wraps `csInterface.evalScript()` in a Promise-based API. Exposes exactly three functions: `init`, `dispatch`, and `dispatchBatch`.
...
- Exposed as a global `uuidGenerator` object — not a class
...
|---|---|---|---|
| `evalBridge.init` | `(cs)` | `void` | Receives the `CSInterface` instance from `index.js` and stores it internally. Also bootstraps the ExtendScript preamble by loading `json.jsx`, `utils.jsx`, and `dispatcher.jsx` into AE via `csInterface.evalScript()`. Must be called before any `dispatch` or `dispatchBatch` call. |
...
`index.js` creates the `CSInterface` instance and passes it to `evalBridge.init(cs)` at panel startup. `init` is responsible for:

1. Storing the `CSInterface` instance internally so `dispatch` and `dispatchBatch` can use it.
2. Bootstrapping the preamble: loading `jsx/json.jsx`, `jsx/utils.jsx`, and `jsx/dispatcher/dispatcher.jsx` into AE in that exact order via `csInterface.evalScript()`. This must complete before any `dispatch` or `dispatchBatch` call is made. The preamble load is fire-and-forget — `init` does not return a Promise. The panel init sequence (in `index.js`) is responsible for ensuring sufficient time or sequencing before the first dispatch.

`json.jsx` must load first — it is the JSON polyfill that all other ExtendScript files depend on.
...
- In the browser (outside AE), `csInterface` is not defined and `init` has not been called
- `dispatch` and `dispatchBatch` must detect this and reject the Promise with a clear message: `'[evalBridge] csInterface not available — panel is running outside After Effects'`
- This guard allows the panel UI to load and render in a browser tab without crashing
- Check with: `typeof csInterface === 'undefined'` or by checking whether the internal stored reference is null
...
var evalBridge = (function() {
  ...
  var _cs = null; // set by init(cs)

  function init(cs) {
    // store cs instance
    // bootstrap preamble: evalScript json.jsx, then utils.jsx, then dispatcher/dispatcher.jsx
  ...
  function dispatch(commandObj) {
    return new Promise(function(resolve, reject) {
      ...
      // call _cs.evalScript(callString, callback)
...
This test runs in a browser tab where `csInterface` is not available and `init` has not been called. It verifies the API surface and the guard behavior — not actual AE communication.
...
(function() {
  ...
  assert('evalBridge.init is function',          typeof evalBridge.init === 'function');
  ...
  // dispatch rejects when csInterface unavailable (browser context, init not called)
...
*TASK-03-UUID-EVALBRIDGE.md — Procedia v4 — May 2026*
*Prerequisite reading: CLAUDE.md Skills 2 & 3, arch_specs.md Section 5*
