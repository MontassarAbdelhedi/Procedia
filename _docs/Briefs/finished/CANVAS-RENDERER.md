...
2. `arch_specs.md` — Sections 8 and 14 in full
...
**This task also extends `graph/graphState.js`** with two new selection methods: `setSelection(uuid)` and `getSelection()`. These are required by both `renderer.js` and `input.js` and must be added before those files are written. See Phase 0.
...
## PHASE 0 — Extend `graph/graphState.js` with selection API

### Why this phase exists

`renderer.js` calls `graphState.getSelection()` to determine whether to apply the `selected` class to a node card. `input.js` calls `graphState.setSelection(uuid)` when the user clicks a node or the canvas background. Neither `structure.md` nor the current `graphState.js` public API lists these methods — they must be added before any canvas file is written.

### What to add

Add a private `_selectedNodeId` variable (initialised to `null`) and two public functions to `graphState.js`:

```javascript
// Inside graphState.js — add alongside the existing public API

var _selectedNodeId = null;

function setSelection(uuid) {
  _selectedNodeId = uuid || null;
  if (_onSelectionChange) _onSelectionChange(_selectedNodeId);
}
...
// Callback hook — registered externally (e.g. by renderer or index.js)
...
Expose all three on the returned public object: `setSelection`, `getSelection`, `onSelectionChange`.

### Update `structure.md`

After adding these methods, update the `graphState.js` entry in `structure.md` to include:

```
setSelection(uuid), getSelection(), onSelectionChange(fn)
```

in the `Exposes:` line alongside the existing public API list. `structure.md` is the load-order and public-API truth — it must never fall behind the actual file.
...
```javascript
graphState.setSelection('test-uuid');
console.assert(graphState.getSelection() === 'test-uuid', 'setSelection/getSelection round-trip');
graphState.setSelection(null);
console.assert(graphState.getSelection() === null, 'setSelection null clears');
console.log('[Phase 0] selection API ok');
```

Zero assertion failures required.

**STOP. Paste console output. Wait for confirmation before Phase 1.**
...
var viewport = (function() {
  ...
  function setZoom(newZoom, originX, originY) {
    // zoom-to-origin logic — fill from formula above
  }
...
Fill `setZoom` completely from the formula above. Everything else is provided — copy it exactly.
...
Every node card must use this exact structure and class names.
...
**Dynamic params:** Effector nodes with `params: 'dynamic'` do not have a static `def.params` array. Read `nodeData.dynamicSchema` instead. If `dynamicSchema` is null or has no properties, render a loading placeholder (`<span class="node-param-loading">Loading…</span>`) in the node body. When `dynamicSchema` is present, iterate `dynamicSchema.properties` and render each property using `property.matchName` as the key and `property.label` as the display label — the same display rules apply as for static params.
...
(function devBootstrap() {
  ...
  // Resolve node definitions from the registry
  ...
  // Drop nodes — engine.dropNode expects the full node definition object, not a type string
  var n1 = engine.dropNode(textDef, 120, 140);
  console.log('[dev] dropped text node:', n1 && n1.id);

  var n2 = engine.dropNode(compDef, 400, 140);
  console.log('[dev] dropped comp node:', n2 && n2.id);

  var n3 = engine.dropNode(nullDef, 120, 320);
...
- [ ] Parent port dots are visible on top/bottom edges of Text and Null nodes
- [ ] No console errors
...
Also wire selection changes to the renderer. In `index.js` (or at the bottom of `renderer.js` init):
...
**Dynamic params (`params: 'dynamic'`):** The renderer must handle nodes whose `def.params` is the string `'dynamic'` rather than an array. In that case, read `nodeData.dynamicSchema` and render from its `properties` array. If `dynamicSchema` is null, render a loading placeholder. Never attempt to iterate the string `'dynamic'` as an array.
...
graph/graphState.js         ✅  setSelection / getSelection / onSelectionChange added
...
structure.md updated: graphState.js public API reflects new selection methods.
...
*Prerequisite reading: CLAUDE.md Skills 11, 12, 13 — arch_specs.md Sections 8 and 14*
