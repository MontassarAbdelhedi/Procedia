...
[x] In-app update system (`updater/` — core.js, nodeAdapter.js, updateService.js, helper.ps1): checks stable `latest.json` feed on GitHub raw; semver validation + AE/updater compatibility gates; 24h auto-check throttle with manual bypass; download restricted to trusted HTTPS GitHub hosts; streamed SHA-256 verification; external PowerShell helper inspects every ZIP entry (rejects traversal/absolute paths/symlinks), extracts to staging, validates package (extension ID, version, entry point, manifest, dispatcher, script manifest, helper), waits for all AfterFX processes to exit, swaps install with one backup + rollback on failure; "Restart After Effects" flow; healthy-start confirmation removes backup; persistent state in `%APPDATA%\Uppercut Studio\Procedia\updater-state.json`
[x] Settings modal Updates tab (`ui/settingsModal/updates.js` + dom/index/css): installed/latest version hero, status line with data-state coloring, determinate/indeterminate progress bar, Check/Install/Retry/View-release-notes buttons; `settingsModal.open(tabName)` + `openUpdates()` API added
[x] Top-bar update badge (`ui/topBar/dom.js`, `events.js`, `css/topBar.css`): glowing dot shown when update available/ready/restart-required; click opens Settings directly to Updates tab
[x] Release tooling: `scripts/package-release.js` (dist ZIP via Compress-Archive + SHA-256 checksum file + `latest.json` generation); `scripts/sync-version.js` (`npm run version:sync` — syncs CSXS manifest, installer.iss, envSnapshot/reporter fallbacks from package.json as single version authority); `npm run release` pipeline (version:sync → test → build → package); `scripts/installer.iss` Inno Setup 6 per-user fresh-install EXE; `_docs/update-system.md` full runtime design + publishing procedure; RELEASING.md rewritten to point at it
[x] Updater tests: tests/updater/{core,service,ui}.test.js + tests/integration/updater-packaging.test.js (version alignment across package/manifest/installer, scripts.json load order for updater deps, CEP Node enabled + AE 2020+ manifest contract)
[x] Fix deleteNode: alive effector hosted in active comp now bypasses cascade — input wire re-routed directly to downstream target instead of ghosting the whole path; keeps existing direct path if present (`_bypassActiveCompEffector` in graph/engine/nodes/deleteNode.js)
[x] Fix wire-insertion restamp race in propagate.js: `restampLayer` dispatch now awaited before firing transplant onAlive command; effector/blending transplant commands get `layerNodeUUID = pathLayerUUID`; restamp failure marks node state 'error'
[x] Fix findPathLayerUUID memoized cache staleness: helpers/index.js now wraps ALL graphState mutation methods (addNode/removeNode/updateNode/batchUpdateNodes/addWire/removeWire/updateWire/loadGraph/clearGraph/_replaceState/replaceGraph) with cache invalidation — not just rebuildTempGraph (CRUD modules call internal rebuilds directly)
[x] Startup chain hardening (index.js): ensureReservedComp/readGraph/versionControl failures now throw and surface instead of silently continuing; startup.log relocated to `%APPDATA%\Uppercut Studio\Procedia\logs\`
[x] Per-user writable data migration off the replaceable extension folder: new `_procediaDataFolder`/`_procediaDataSubfolder` in jsx/utils.jsx; schema cache moved to `%APPDATA%/Uppercut Studio/Procedia/cache/effectSchemaCache.json` (legacy copy-on-read + atomic tmp+rename write); graphExport diagnostics to `diagnostics/` subfolder; cmd-chunk temp files to OS temp `/Procedia`; writeTextFile writes under per-user data root
[x] XSS hardening: nodeList/render.js + nodePicker/render.js escape node labels/types/category names; dragdrop drag ghost built via DOM APIs + textContent instead of innerHTML
[x] Preset name validation (presetManager.js): names >80 chars or containing `<>"'`` rejected on save and skipped on load
[x] Manifest compatibility widened: CSXS manifest Version 11.0→7.0, HostList [24.0,99.9]→[17.0,99.9] (AE 2020+); `--enable-nodejs` CEF parameter added (updater requirement); disable-web-security/allow-file-access-from-files params removed
[x] envSnapshot version read: relative manifest URL instead of absolute extension path (survives updater swap), attribute-form regex; sync-version injects fallback literal into envSnapshot + reporter/core.js
[x] Build secrets config moved `.debug/build.config.json` → `.secrets/build.config.json` (`.debug` file reserved for CEP remote debugging); build.js excludes more root entries from bundle (dist, scripts, docs, package files)
[x] README updated: AE 2020+, .secrets note; test suite now 296 passing across 20 files

## Sun 2026-08-09

[x] Version control system: introduce versioning/ module — repositoryStore.js (branches/revisions/artifacts/invariants), branchService.js, revisionService.js, capabilities.js; snapshot/ subdirectory (canonicalizer, checksum, migrations, schema, serializer — 5 files); diff/semanticDiff.js; merge/ subdirectory (conflictFactory, conflictResolver, mergeBase, mergeValidator, threeWayMerge — 5 files); activation/ subdirectory (activationCoordinator, graphSyncPlanner — 2 files); versionControlService.js public API aggregator — repositoryStore.js (branches/revisions/artifacts/invariants), branchService.js, revisionService.js, capabilities.js; snapshot/ subdirectory (canonicalizer, checksum, migrations, schema, serializer — 5 files); diff/semanticDiff.js; merge/ subdirectory (conflictFactory, conflictResolver, mergeBase, mergeValidator, threeWayMerge — 5 files); activation/ subdirectory (activationCoordinator, graphSyncPlanner — 2 files); versionControlService.js public API aggregator
...
[x] Large-file refactoring (panel side): bridge/evalBridge.js → extracted allowedActions.js + jsxFiles.js (2 files); graph/engine/helpers.js → helpers/{buildInitialProps,refreshUI,pathLayer,dynamicSchema,expressionDispatch,dataPropagation,deepCopyNode,index}.js (8 files); flush/dirtyFlusher.js → flush/{flushNode,pathLayerUtil}.js + barrel kept (2 files)
[x] data/scripts.json manifest updated: 238→249 entries (11 new split files)
