...
- **Node definitions:** Loaded by `graph/nodes/loadNodes.js` via `document.write()` — non-effect nodes (25 files) loaded synchronously, effect nodes (460+) generated on-demand by `graph/engine/effectNodeFactory.js` from metadata stubs under `graph/nodeMetadata/` (22 category files).
...
- **Node UUIDs:** Node `PROC-{timestamp}-{rand4}`, wire `WIRE-{timestamp}-{rand4}`, comment `CMT-{timestamp}-{rand4}`. Layer `.comment` = terminal wire UUID, never node UUID. Comp `.comment` = node UUID (CompNode only).
...
- **Adding a new node:** Create one file under `graph/nodes/categories/`, register in `nodeRegistry`, add its `<script>` tag to `graph/nodes/loadNodes.js` and its path to `data/scripts.json`. Adding a new AE action requires editing `jsx/dispatcher/dispatcher.jsx` and adding the action to the `_ALLOWED_ACTIONS` whitelist in `bridge/evalBridge.js`.
